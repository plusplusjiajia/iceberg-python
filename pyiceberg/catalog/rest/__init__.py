    ...
    def _init_sigv4(self, session: Session) -> None:
        ...
        class _IcebergSigV4Auth(SigV4Auth):
            def canonical_request(self, request: AWSRequest) -> str:
                # Force the hex payload hash in the canonical request even when the
                # x-amz-content-sha256 header is base64 (set in SigV4Adapter.add_headers).
                # Mirrors botocore <=1.42.x SigV4Auth.canonical_request layout, and keeps
                # byte-parity with Iceberg Java's RESTSigV4AuthSession.
        ...
        class SigV4Adapter(HTTPAdapter):
            ...
            def add_headers(self, request: PreparedRequest, **kwargs: Any) -> None:  # pylint: disable=W0613
                ...
                # The SigV4 canonical request still uses hex (enforced in _IcebergSigV4Auth).
                if request.body:
                ...
                if AUTHORIZATION_HEADER in signing_headers:
                    signing_headers[f"Original-{AUTHORIZATION_HEADER}"] = signing_headers.pop(AUTHORIZATION_HEADER)
